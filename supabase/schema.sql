...
create table if not exists public.vsphere_profiles (
...
create table if not exists public.vsphere_posts (
  id bigint generated always as identity primary key,
  user_id uuid not null references auth.users(id) on delete cascade,
  category text not null default 'free' check (category in ('free', 'clip', 'promo', 'notice')),
  title text not null,
...
alter table public.vsphere_profiles enable row level security;
alter table public.vsphere_posts enable row level security;

drop policy if exists "Anyone can read public profiles" on public.vsphere_profiles;
drop policy if exists "Users can update own profile" on public.vsphere_profiles;
drop policy if exists "Anyone can read published posts" on public.vsphere_posts;
drop policy if exists "Users can create own posts" on public.vsphere_posts;
drop policy if exists "Users can update own posts" on public.vsphere_posts;
drop policy if exists "Users can delete own posts" on public.vsphere_posts;

create policy "Anyone can read public profiles"
on public.vsphere_profiles
for select
to anon, authenticated
using (true);
...
create or replace function public.vsphere_handle_new_user()
returns trigger
language plpgsql
security definer set search_path = public
as $$
begin
  insert into public.vsphere_profiles (id, email, display_name, avatar_url)
  values (
    new.id,
    new.email,
...
drop trigger if exists vsphere_on_auth_user_created on auth.users;

create trigger vsphere_on_auth_user_created
after insert on auth.users
for each row execute procedure public.vsphere_handle_new_user();
